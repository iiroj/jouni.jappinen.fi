...
const STATIC_FILE = /\.(?:jpg|pdf|png|svg|webmanifest)$/
...
exports.handler = (event, context, callback) => {
    ...
    const addCacheControl = (value) => void (response.headers['cache-control'] = [{ key: 'Cache-Control', value }])
    ...
    if (HTML_CONTENT_TYPE.test(contentType) || HTML.test(uri)) {
        addCacheControl('public,max-age=0,must-revalidate,s-maxage=2592000,stale-while-revalidate')
    }
